...
impl IntoResponse for AuthError {
    fn into_response(self) -> axum::response::Response {
        match self {
            ...
            AuthError::FormatError => (http::StatusCode::UNAUTHORIZED, "Unauthorized").into_response(),
    ...
    S: Send + Sync,
{
    ...
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        ...
        if let Ok(user_token_data) = jwt_processor.validate(&token) {
            return Ok(Self { claims: user_token_data.claims });
        }

        jwt_processor = jwt_processor.algorithm(token_data.header.alg);
        let device = devices::Model::find_device(&ctx.db, &token_data.claims.identity)
            .await
            .map_err(|_| AuthError::Unauthorized)?;

        if let Ok(token_data) = jwt_processor.validate_pem(&token, device.public_key.as_bytes()) {
            return Ok(Self { claims: token_data.claims });
        }
...
fn extract_token(jwt_config: &JWTConfig, parts: &Parts) -> Result<String, Error> {
    ...
    extract_token_from_query(TOKEN_PREFIX, parts)
        .or_else(|_| {
